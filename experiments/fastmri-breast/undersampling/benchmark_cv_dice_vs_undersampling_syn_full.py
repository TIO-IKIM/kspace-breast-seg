"""Evaluate full-dataset CV models (trained on SYNTHETIC k-space) under
various undersampling rates and k-space noise levels.
...
OUT_DIR = f"{RESULTS_BASE}/undersampling_benchmark_cv_syn_full"
BATCH_SIZE = 16
PATCH_DEPTH = 24
PATCH_STRIDE = 16
NUM_WORKERS = 2
...
def main():
    ...
    for fold_idx, split in enumerate(splits, start=1):
        ...
        for model_name, (cv_suffix, model_class) in MODELS.items():
            ckpt_dir = f"{RESULTS_BASE}/3d_cv_{cv_suffix}_syn_full/fold_{fold_idx}/{model_class}/checkpoints"
    ...
    ax.set_title("Dice vs Undersampling (syn)")
    ...
    # axes[1].set_title("AUROC vs Undersampling (syn)")
    ...
    ax_n.set_title("Dice vs K-space Noise (syn)")
    ax_n.invert_xaxis()
