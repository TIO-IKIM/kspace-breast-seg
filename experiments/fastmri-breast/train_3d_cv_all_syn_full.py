...
using the SYNTHETIC (image-space) k-space preprocessing from
`datasets/fastMRI/prepare_kspace_syn_full.py` → `training_data_syn_full/`.

Identical hyper-parameters to train_3d_cv_all_full.py; only the dataset
directory (and output/run names) differ.
"""
import csv
import os
...
class CombinedEarlyStopping(pl.Callback):
    """Stop training only when ALL monitored metrics have plateaued."""

    def __init__(self, monitors, patience=15, mode="max"):
        super().__init__()
        self.monitors = monitors
        self.patience = patience
        self.mode = mode
        self._best = {m: None for m in monitors}
        self._wait = {m: 0 for m in monitors}
    ...
    def on_validation_end(self, trainer, pl_module):
        ...
        for m in self.monitors:
            ...
            if val is None:
                continue
            current = val.item() if hasattr(val, "item") else float(val)
            if self._is_improvement(current, self._best[m]):
                self._best[m] = current
                self._wait[m] = 0
            else:
                self._wait[m] += 1

        if all(self._wait[m] >= self.patience for m in self.monitors):
...
def train(model_name: str):
    ...
    dataset_dir = "/home/l721f/data/fastmri-breast/training_data_syn_full"
    output_dir = f"/home/l721f/code/kspace-pred-net/results/fastmri-breast/3d_cv_{model_name}_syn_full/"
    ...
    trainer_kwargs = dict(
        ...
        callbacks=[CombinedEarlyStopping(
            monitors=["val_vol_dice_class_1_pos"],
            patience=15,
            mode="max",
        )],
    )
    ...
    run_cv_3d_experiment(
        ...
        run_name_base=f"{model_name}-3d-cv-syn-full",
        project_name="kspace-pred-net-fastmri-breast-cv-syn-full",
